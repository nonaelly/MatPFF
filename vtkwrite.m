...
%
%  [Wang Zhetong]: Add some new element types.
...
switch upper(dataType)
    ...
    case {'STRUCTURED_GRID','UNSTRUCTURED_GRID'}
        ...
        % Add CELLS and CELL_TYPES
        if strcmpi(dataType,'UNSTRUCTURED_GRID')
            % Parse remaining argument.
            cidx = find(strcmpi(varargin,'CELLS'));
            ctidx = find(strcmpi(varargin,'CELL_TYPES'));
            if cidx~=0
                for ii = 1:length(cidx)
                    element = varargin{cidx(ii)+1};
                    size_element = size(element,1)*(size(element,2)+1);
                    % Data enteries begin with a keyword specifying data type
                    % and numeric format.
                    fprintf(fid, '\nCELLS %d %d\n', size(element,1), size_element);
                end
            end
            if ctidx~=0
                for ii = 1:length(ctidx)
                    elemType = varargin{ctidx(ii)+1};
                    switch elemType
                        case 12
                            fprintf(fid,'8 %d %d %d %d %d %d %d %d\n',(element-1)');
                        case 24
                            fprintf(fid,'10 %d %d %d %d %d %d %d %d %d %d\n',(element-1)');
                        case 23
                            fprintf(fid,'8 %d %d %d %d %d %d %d %d\n',(element-1)');
                        case 9
                    ...
                    fprintf(fid,'\nCELL_TYPES %d\n',size(element,1));
                    Type = elemType*ones(size(element,1),1);
                    fprintf(fid,'%d\n',Type);
                end
            end
        end
